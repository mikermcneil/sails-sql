...
var getIsValidConnection = require('./private/get-is-valid-connection');
var helpGetConnection = require('./private/help-get-connection');
var helpReleaseConnection = require('./private/help-release-connection');
var helpSendNativeQuery = require('./private/help-send-native-query');
...
module.exports = buildWaterlineAdapter({
  ...
  createManager: async function({connectionString, onUnexpectedFailure, meta}) {
    ...
    try {
      ...
      let error = flaverr({ message: 'Provided value ('+connectionString+') is not a valid MySQL connection string.  ' + err.message }, err);
  ...
  getConnection: async function({manager, meta}) {
    ...
    // reject({failed: report});
    try {
      return helpGetConnection(manager, meta);
    } catch (err) {
      if (flaverr.taste('E_FAILED', err)) {
        let report = { error: err.raw, meta };
        throw {failed: report};
      } else {
        throw err;
      }
  ...
  releaseConnection: async function({connection, meta}) {
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    try {
      return helpReleaseConnection(connection, meta);
    } catch (err) {
      if (flaverr.taste('E_BAD_CONNECTION', err)) {
        let report = { meta };
  ...
  sendNativeQuery: async function({connection, nativeQuery, valuesToEscape, meta}) {
    ...
    switch (connection.dialect) {
      case 'mysql':
        if (!getIsValidConnection(connection)) { throw {badConnection: {meta}}; }//•
   ...
   * @see http://npmjs.com/package/driver-interface
   */
  parseNativeQueryError: async function({connection, TODO, meta}) {
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // ^^FUTURE: Breaking change: Instead of `meta` as input, use `this` context
    // to take advantage of runner's natural support for this through `.meta()`
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   ...
   * @see http://npmjs.com/package/driver-interface
   */
  beginTransaction: async function({connection, TODO, meta}) {
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // ^^FUTURE: Breaking change: Instead of `meta` as input, use `this` context
    // to take advantage of runner's natural support for this through `.meta()`
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   ...
   * @see http://npmjs.com/package/driver-interface
   */
  commitTransaction: async function({connection, TODO, meta}) {
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // ^^FUTURE: Breaking change: Instead of `meta` as input, use `this` context
    // to take advantage of runner's natural support for this through `.meta()`
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   ...
   * @see http://npmjs.com/package/driver-interface
   */
  rollbackTransaction: async function({connection, TODO, meta}) {
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // ^^FUTURE: Breaking change: Instead of `meta` as input, use `this` context
    // to take advantage of runner's natural support for this through `.meta()`
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
