...
module.exports = buildWaterlineAdapter({
  ...
  destroyManager: async function({manager, meta}) {
    ...
    // to take advantage of runner's natural support for this through `.meta()`
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    var dialect = 'mysql';// TODO: negotiate dialect
    switch (dialect) {
      case 'mysql':
  ...
  releaseConnection: async function({connection, meta}) {
    ...
    switch (dialect) {
      case 'mysql':
        if (!getIsValidConnection(connection, dialect)) { throw {badConnection: {meta}}; }//•
        try {
          connection.release();
        ...
        // IWMIH, we got rid of the connection, one way or another.
        return ({ meta });
