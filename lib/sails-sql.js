...
module.exports = buildWaterlineAdapter({
  ...
  createManager: async({connectionString, onUnexpectedFailure, meta})=>{
    ...
    try {
      ...
      // TODO: bring in fancier version of this code (believe it's in sails-mysql or sails-mongo)
   ...
   * @see http://npmjs.com/package/driver-interface
   */
  destroyManager: async({manager, unusedMeta})=>{
    ...
    .intercept('E_FAILED', (err)=>( {failed: err} ));
  },
