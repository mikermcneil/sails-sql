...
module.exports = function compileWhereClauseIntoKnexChain (whereClause, kChain, unusedDryWLModel, unusedDryOrm){
  ...
  console.log('s3q "where" clause:', require('util').inspect(whereClause,{depth:null}));
  ...
  if (_.keys(whereClause).length > 0) {
    (function $recurse(scruple, kChain, recursionDepth, disjunctMode, scrupleIdx){
      ...
      console.log('scrupleKey',scrupleKey, disjunctMode?'(disjunct mode)':'', scrupleIdx!==undefined?(' (scruple idx: '+scrupleIdx+')'):'');
      ...
      if (scrupleKey !== 'and' && scrupleKey !== 'or') {
        ...
        // > little curious and repetitive, check out these commits:
        // > • https://github.com/mikermcneil/sails-sql/commit/536bcb18e485b00ba4f7bfe2ec702bfd3b331b2d#commitcomment-31424796
        // > • https://github.com/mikermcneil/sails-sql/commit/ec35d83842cd5cce4bae13be5bfdec9941d6a924
        // > (^i.e. changed it to work the way it is now for simplicity and to avoid using `.bind()`)
        ...
        //    > Note: If you try to just do `(disjunctMode ? kChain.orWhere : kChain.andWhere)`
        //    > by itself  here, you get an error from Knex:  "Cannot read property 'whereWrapped' of undefined"
        //    > That's why we're messing with this whole scrupleIdx business.
        //    > …but apparently you still get it, even then:
        //    > (!scrupleIdx || scrupleIdx === 0 ? kChain.where : disjunctMode ? kChain.orWhere : kChain.andWhere)
        //    >
        //    > So we need something else still!  This seems to be because of `this` binding (it's all coming back to me)
        //    > So... the only way to do this concisely is to pull out an inline function declaration.
        //    >
        //    > Gross.
        //    >
        let handlePredicate = (parenthetical)=>{
          for (let i=0; i<conjunctsOrDisjuncts.length; i++) {
            $recurse(conjunctsOrDisjuncts[i], parenthetical, recursionDepth+1, scrupleKey === 'or', i);
          }//∞
        };//ƒ
        if (disjunctMode) {
          kChain.orWhere(handlePredicate);
        } else {
          kChain.andWhere(handlePredicate);
        }
