...
var DRIVER = {
  ...
  createManager: async function({connectionString, onUnexpectedFailure, meta}) {
    ...
    // MySQL's `socketPath` is automatically supported.
  ...
  createRecord: async function({query, connection, dryOrm}) {
    ...
    var phRecords;
    try {
      phRecords = await DRIVER.createEachRecord({ query: queryCopy, connection, dryOrm });
    } catch (errOrSignal) {
      console.log('ERR FROM TRYING TO CALL CREATEEACHRECORD INTERNALLY:', errOrSignal);
      throw errOrSignal;
    }
