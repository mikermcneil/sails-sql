...
var requireDbLibrary = (dialect)=>{
  switch (dialect) {
    ...
    // TODO: since only one request can be handled at a time per connection
    // in MSSQL, we'll need to change the implementation so far to map our
    // concept of a "connection" to a connection pool-- with some extra stuff
    // to handle transactions (because they still actually _do_ need to be run
...
var DRIVER = {
  ...
  createManager: async function({connectionString, onUnexpectedFailure, meta}) {
    ...
    if (dialect === 'mysql') {
      ...
      let pool = new dbLibrary.ConnectionPool(connectionString);

      // Handle unexpected failures (note that we bind the event handler, no matter what.)
      // For more background, see:
      //  • https://www.npmjs.com/package/mssql/v/4.3.1#connections-1
      pool.on('error', (err)=>{
        if (onUnexpectedFailure) {
          let message = `One or more pooled connections to this ${dialect} database were lost. Did the database server go offline?  ${(err?err.message:'')}`;
          if (flaverr.parseError(err)) {
      ...
      return {
        manager: { pool, connectionString, dialect, dbLibrary, unreleasedConnections: [] },
        meta
      };
  ...
  destroyManager: async function({manager, meta}) {
    ...
    if (manager.dialect === 'mysql') {
      ...
      manager.pool.removeAllListeners('error');
      try {
        // Before closing the pool, all ongoing "connections" must be released.
        ...
        // >  • https://github.com/tediousjs/node-mssql/blob/v4.3.1/lib/base.js#L280-L299
        // >  • https://www.npmjs.com/package/mssql/v/4.3.1#close
  ...
  getConnection: async function({manager, meta}) {
    ...
    if (manager.dialect === 'mysql') {
      ...
      try {
        // https://github.com/tediousjs/node-mssql/blob/v4.3.1/lib/base.js#L164-L187
        let underlyingConnection = await manager.pool.acquire();
        let connection = {
        ...
        let error = flaverr({
          message: 'Failed to obtain a SQLServer database connection from the pool.  '+err.message
        }, err);
        throw {failed: { error, meta }};
      }
    } else if (manager.dialect === 'oracledb') { throw new Error('Support for that dialect is incomplete...  (FUTURE)'); }
    else if (manager.dialect === 'sqlite3') { throw new Error('Support for that dialect is incomplete...  (FUTURE)'); }
  ...
  releaseConnection: async function({connection, meta}) {
    ...
    if (connection.dialect === 'mysql') {
      ...
      // https://github.com/tediousjs/node-mssql/blob/v4.3.1/lib/base.js#L189-L203
      manager.pool.release(connection.underlyingConnection);
      return { meta };
   ...
   *
   * --OR--
   * TODO: at least convert the stuff from knex into $1, $2, etc.
   * Currently we're just using ? everywhere.
   * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   */
  sendNativeQuery: async function({connection, nativeQuery, valuesToEscape, meta}) {
    ...
    try {
      if (connection.dialect === 'mysql') {
        ...
        let mssqlRequest = connection.manager.pool.request();
        let potentiallyAdjustedNativeQuery;
        if (!valuesToEscape) {
          potentiallyAdjustedNativeQuery = nativeQuery;
        } else {
          let i = 0;
          let potentiallyAdjustedNativeQuery = nativeQuery.replace(/\?/g, ()=>{
          ...
          // FUTURE: maybe switch to tedious?
          for (let idx in valuesToEscape) {
            // TODO: other types?  (https://github.com/tediousjs/node-mssql#input-name-type-value)
            mssqlRequest = mssqlRequest.input(`p${idx}`, connection.manager.dbLibrary.NVarChar, valuesToEscape[idx]);
          }//∞
        }//•
        let result;
        try {
          result = await mssqlRequest.query(potentiallyAdjustedNativeQuery);
        } catch (err) {
          console.log('GOT ERROR FROM ATTEMPTING TO RUN QUERY:', err);
          // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
          // TODO: figure out where we're not getting an error for "SELECT * FROM notarealtable"
          //
          // ^^^ if can't find a clean explanation here, just rip mssql out and use tedious instead
          // e.g. https://github.com/tgriesser/knex/blob/b56ff04e610140d9554ec609986831cd7936960a/src/dialects/mssql/index.js#L62
          // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
          throw {queryFailed: {error: err, meta}};
        }
        return { result, meta };
        // ------ Trying it w/ callbacks to see if that makes it work better
        // return await new Promise((resolve, reject)=>{
        //   mssqlRequest.query(potentiallyAdjustedNativeQuery, (err, result)=>{
        //     if (err) {
        //       console.log('GOT ERROR FROM ATTEMPTING TO RUN QUERY:', err);
        //       return reject({queryFailed: {error: err, meta}});
        //     }//•
        ...
        //---- OLD APPROACH:
        // return await new Promise((resolve, reject)=>{
        //   let mssqlRequest = (new connection.manager.dbLibrary.Request(adjustedNativeQuery, (err, result)=>{
        //     if (err) {
        //       reject({queryFailed:{error:err, meta}});
        //     } else {
        //       resolve({ result, meta });
        //     }
        //   }));//_∏_
        //   for (let idx in valuesToEscape) {
        //     // TODO: other types?
        //     mssqlRequest.addParameter(`p${idx}`, connection.manager.dbLibrary.NVarChar, valuesToEscape[idx]);
        //   }//∞
        //   connection.underlyingConnection.execSql(mssqlRequest);
        // });//•
        // throw new Error('Support for that dialect is incomplete...  (FUTURE)');
      } else if (connection.dialect === 'oracledb') { throw new Error('Support for that dialect is incomplete...  (FUTURE)'); }
      else if (connection.dialect === 'sqlite3') { throw new Error('Support for that dialect is incomplete...  (FUTURE)'); }
  ...
  parseNativeQueryError: function({nativeQueryError, meta}) {
    ...
    if (nativeQueryError.dialect === 'mysql') {
      ...
      let footprint;
      console.log('PARSING AN ERROR!!!!!', Object.keys(nativeQueryError), nativeQueryError);
      if (flaverr.taste({code: 'EREQUEST', number: 3701}, nativeQueryError)) {// https://docs.microsoft.com/en-us/previous-versions/sql/sql-server-2008-r2/cc645601%28v%3dsql.105%29
        footprint = { identity: 'noSuchPhysicalModel' };
      } else {// TODO: handle uniqueness errors
        footprint = { identity: 'catchall' };
      }
      return { footprint, meta };
  ...
  createEachRecord: async function({query, connection, dryOrm}) {
    ...
    for (let qInfo of nativeQueries) {
      let nativeResult;
      try {
        nativeResult = await runNativeQuery(qInfo.sql, qInfo.bindings, connection, query.meta, DRIVER);
      } catch (err) {
      ...
      if (isFetchEnabled) {
        if (isCapableOfOptimizedReturnTactic) {
          ...
          throw new Error('FUTURE: Not yet implemented optimized "returning" tactic for dbs like postgresql and mssql - see nativeResult: '+nativeResult);
        } else {
          insertIds.push(nativeResult.insertId);
        }
      }
    }//∞
  ...
  updateRecords: async function({query, connection, dryOrm}) {
    ...
    var nativeResult;
    try {
      nativeResult = await runNativeQuery(kQuery.sql, kQuery.bindings, connection, query.meta, DRIVER);
    } catch (err) {
    ...
    if (isCapableOfOptimizedReturnTactic) {
      ...
      throw new Error('FUTURE: Not yet implemented optimized "returning" tactic - see native result: '+nativeResult);
    }
  ...
  destroyRecords: async function({query, connection, dryOrm}) {
    ...
    var nativeResult = await runNativeQuery(kQuery.sql, kQuery.bindings, connection, query.meta, DRIVER);
    if (isCapableOfOptimizedReturnTactic) {
  ...
  findRecords: async function({query, connection, dryOrm}) {
    ...
    if (query.select === undefined) {
      ...
      kChain = kChain.select(query.select);
    }
    ...
    switch (dialect) {
      ...
      case 'mssql':
        rows = nativeResult.recordsets;// TODO
        console.log('native result for find:', nativeResult);
        break;
  ...
  countRecords: async function({query, connection, dryOrm}) {
    ...
    var kQuery = kChain.toSQL();
    var nativeResult = await runNativeQuery(kQuery.sql, kQuery.bindings, connection, query.meta, DRIVER);
    var total = nativeResult.rows[0].VAL;
    if (dialect === 'pg') {
  ...
  sumRecords: async function({query, connection, dryOrm}) {
    ...
    var kQuery = kChain.toSQL();
    var nativeResult = await runNativeQuery(kQuery.sql, kQuery.bindings, connection, query.meta, DRIVER);
    var sum = nativeResult.rows[0].VAL;
  ...
  avgRecords: async function({query, connection, dryOrm}) {
    ...
    var kQuery = kChain.toSQL();
    var nativeResult = await runNativeQuery(kQuery.sql, kQuery.bindings, connection, query.meta, DRIVER);
    var arithmeticMean = nativeResult.rows[0].VAL;
