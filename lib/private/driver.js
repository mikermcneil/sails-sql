...
var DRIVER = {
      ...
      if (connection.dialect === 'mysql') {
        ...
        if (!valuesToEscape) {
          ...
          for (let idx in valuesToEscape) {
            ...
            mssqlRequest.input(`p${idx}`, connection.manager.dbLibrary.NVarChar, valuesToEscape[idx]);
          }//∞
        }//•
        let result;
        try {
  ...
  findRecords: async function({query, connection, dryOrm}) {
    ...
    var nativeResult = await runNativeQuery(kQuery.sql, kQuery.bindings, connection, query.meta, DRIVER);
    processNativeRecords(nativeResult.rows, dialect, DryWLModel.identity, dryOrm);
