...
var DRIVER = {
  ...
  createEachRecord: async function({query, connection, dryOrm}) {
    ...
    for (let phValuesToSet of query.newRecords) {
      _reifyPhysicalValuesToSet(phValuesToSet, dialect, DryWLModel);
    }//∞
    ...
    if (isFetchEnabled) {
      _processNativeRecords(phRecords, dialect, DryWLModel.identity, dryOrm);
      return phRecords;
    }//ﬁ
  },
...
function _reifyPhysicalValuesToSet (phValuesToSet, dialect, DryWLModel){
...
function _processNativeRecords (phRecords, dialect, parentModelIdentity, dryOrm){
      ...
      switch (dialect) {
        case 'mysql':
          if (attrDef.type === 'boolean') {
            ...
            // TODO: probably also handle empty string
          }//ﬁ
          break;
        case 'pg':
