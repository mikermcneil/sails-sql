...
var DRIVER = {
  ...
  sendNativeQuery: async function({connection, nativeQuery, valuesToEscape, meta}) {
    ...
    // FUTURE: Maybe do something like this, if it's enough of a problem:
    // (But note this comes with its own potential problems, so we should proceed carefully)
    //
    // // Log a warning if likely-invalid usage is detected
    // if (valuesToEscape && valuesToEscape.length > 0 && nativeQuery.match(/\?/)) {
    //   console.warn(`sails-sql adapter detected unusual usage ("?", i.e. question marks) in your SQL query:\n  ${nativeQuery}\n  But SQL queries with dynamic data should use syntax like "$1", "$2", etc. instead of "?".  For more information, see sailsjs.com/docs/reference/waterline-orm/datastores/send-native-query`);
    // }
