...
var DRIVER = {
  ...
  createManager: async function({connectionString, onUnexpectedFailure, meta}) {
    ...
    // Validate & parse connection string + `meta`, pulling out client config, and calling `malformed` if invalid.
    // > Remember: connection string takes priority over `meta` in the event of a conflict.
    //
    // In this case, we also use this opportunity to determine the dialect.
    var dialect;
    try {
      // TODO: Thoroughly validate/parse/merge URL & `meta` (maybe use generic impl from sails-mongo, but put it in wl-utils -- see branch of wl-utils for more info)
      ...
      // After obtaining a normalized connection string (and potentially normalizing `meta` in place), determine the dialect.
      let connectionStringProtocol = url.parse(connectionString).protocol;
      let hasCustomProtocol = connectionStringProtocol !== 'db:';
      if (hasCustomProtocol) {
        dialect = connectionStringProtocol.replace(/\:$/, '');
      } else if (meta.dialect) {
        dialect = meta.dialect;
      } else {
        throw new Error('Could not determine SQL dialect to use.  Please specify a dialect via the connection URL protocol (e.g. "mysql://").');
      }
    } catch (err) {
      let report = {
        error: err,
    ...
    switch (dialect.toLowerCase()) {
      ...
      case 'oracledb': case 'oracle': dialect = 'oracle'; break;
    ...
    switch (dialect) {
      case 'mysql':
        ...
        let mysql = getLibrary(dialect);
        let underlyingDbClientConfig = meta;
        let pool = mysql.createPool(underlyingDbClientConfig);
      ...
      default:
        let report = {
          error: new Error(`Unrecognized SQL dialect ${dialect}.  Please specify a supported dialect instead; e.g. "mysql", "pg", "mssql", "oracledb", or "sqlite3".`),
          meta
        };
        throw {malformed: report};
    }
