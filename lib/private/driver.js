...
var DRIVER = {
  ...
  createManager: async function({connectionString, onUnexpectedFailure, meta}) {
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // FUTURE: Validate & parse connection string, pulling out client config (call `malformed` if invalid).
    // > Remember: connection string takes priority over `meta` in the event of a conflict.
    // ```
    // try { … } catch (err) {
    //   let report = {
    //     error: flaverr({ message: `Provided value (${connectionString}) is not a valid MySQL connection string.  ${err.message}` }, err),
    //     meta
    //   };
