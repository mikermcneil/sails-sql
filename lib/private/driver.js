...
var DRIVER = {
   ...
   *
   * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   * TODO: update Sails documentation and node-machine driver interface to show using "?" instead of "$1", etc
  ...
  sendNativeQuery: async function({connection, nativeQuery, valuesToEscape, meta}) {
    ...
    try {
      if (connection.dialect === 'mysql') {
        ...
        if (valuesToEscape) {
          let i = 0;
          let adjustedNativeQuery = nativeQuery.replace(/\?/g, ()=>{
          ...
          // console.log('adjustedNativeQuery',adjustedNativeQuery);
          return await new Promise((resolve, reject)=>{
            // TODO: send parameterized query on connection
          ...
          // TODO: send literal query on connection
          let result;
          return { result, meta };
        }
        // throw new Error('Support for that dialect is incomplete...  (FUTURE)');
