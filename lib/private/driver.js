...
var DRIVER = {
  ...
  createEachRecord: async function({query, connection, dryOrm}) {
    ...
    var nativeQueries = [];
    {
      ...
      // > i.e. "Cannot insert explicit value for identity column in table '……' when IDENTITY_INSERT is set to OFF."
      // > For more, see https://github.com/tgriesser/knex/issues/2262#issuecomment-452065270
      // `SET IDENTITY_INSERT ${schema}.${table} ON; ${sql} SET IDENTITY_INSERT ${schema}.${table} OFF;`
      // ^TODO
      //
      // e.g.
      // ```
    ...
    let nativeQueries = k.schema.createTable(tableName, (kTable)=>{
      ...
      for (let column of columns) {
        if (column.autoIncrement) {
          ...
          if (column.unique) {
            // TODO: figure out a better solution for MSSQL, where this defaults to a VARCHAR(1) for some reason...
            // (currently causes this error, but not until later on, at query-time: https://www.brentozar.com/archive/2019/03/how-to-fix-the-error-string-or-binary-data-would-be-truncated/)
