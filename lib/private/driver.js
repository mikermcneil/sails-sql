...
var DRIVER = {
  ...
  findRecords: async function({query, connection, dryOrm}) {
    ...
    if (dialect === 'mssql' && query.criteria.limit === Number.MAX_SAFE_INTEGER) {
      // SQL Server doesn't like us to use big, crazy limits.  (Causes an error.)
      // But by default, Waterline uses Number.MAX_SAFE_INTEGER to indicate a
      // limit-less .find() query.  So we work around this by directly sniffing
      // for this scenario and then, if we notice it, not attaching any limit
      // at all.
    } else {
