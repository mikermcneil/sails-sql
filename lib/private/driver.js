          ...
          for (let idx in valuesToEscape) {
            ...
            mssqlRequest.addParameter(
              ...
              ( // This implementation re: types is based on https://github.com/tgriesser/knex/blob/30bab697ba77b7a20156775c43fc9538b6698368/src/dialects/mssql/index.js#L306-L321
                // > FUTURE: are other types necessary?  (http://tediousjs.github.io/tedious/api-datatypes.html)
                (_.isNumber(valuesToEscape[idx]))?
                  (valuesToEscape[idx] % 1 === 0)?
                    (valuesToEscape[idx] < -2147483648 || valuesToEscape[idx] > 2147483647)?//« min&max bounds for INT4 values in MSSQL
                      connection.manager.dbLibrary.TYPES.BigInt
                    : connection.manager.dbLibrary.TYPES.Int
                  : connection.manager.dbLibrary.TYPES.Decimal
                : connection.manager.dbLibrary.TYPES.NVarChar
              ),
              valuesToEscape[idx],
              {//«For more info on options, see http://tediousjs.github.io/tedious/api-request.html#function_addParameter
                length: Infinity,//« non-numeric only
                // precision: 38,//« decimals only
                // scale: 10,//« decimals only
              }
            );
          }//∞
