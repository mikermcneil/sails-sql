...
var DRIVER = {
  ...
  createEachRecord: async function({query, connection, dryOrm}) {
    ...
    try {
      nativeQueryResult = await runSqlQuery(sql, connection, query.meta, DRIVER);
    } catch (err) {
...
async function runSqlQuery (sql, connection, meta, DRIVER){
  try {
    return await DRIVER.sendNativeQuery({ nativeQuery: sql, connection, meta });
  } catch (errOrSignal) {
    if (errOrSignal.queryFailed) {
      let report = errOrSignal.queryFailed;
      let footprint = (DRIVER.parseNativeQueryError({ nativeQueryError: report.error, meta })).footprint;
