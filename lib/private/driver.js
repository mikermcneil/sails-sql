...
var DRIVER = {
   ...
   */
  countRecords: async function({query, connection}) {
    var tableName = query.using;
    var dialect = connection.dialect;
    var k = knex({ client: dialect });
    ...
    if (dialect === 'pg') {
      // FUTURE: Normalize COUNT result from Postgres, where the native result
      // from COUNT comes back as a string and not a number.
    }

    var kChain = k(tableName).count('* AS total');
    // TODO: actually handle the WHERE clause
    var kQuery = kChain.toSQL();

    var total;
    try {
      let kQueryResult = await _runSqlQuery(kQuery.sql, kQuery.bindings, connection, query.meta, DRIVER);
      total = kQueryResult.rows[0].total;
    } catch (err) {
      throw flaverr({ message: 'Could not count records.  '+err.message }, err);
    }
