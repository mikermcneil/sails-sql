...
var DRIVER = {
  ...
  verifyModelDef: async function({modelDef}) {
    ...
    if (primaryKeyAttrDef.required !== true && primaryKeyAttrDef.autoMigrations.autoIncrement !== true) {
    ...
    switch (this.dialect) {
      ...
      case 'pg':
        ...
        for (let attrName in modelDef.attributes) {
          ...
          let isBigInt = !!attrDef.autoMigrations.columnType.match(/^BIGINT$/i);
  ...
  createEachRecord: async function({query, connection, dryOrm}) {
        ...
        switch (dialect) {
          ...
          case 'pg':
            if (attrDef.type === 'json') {
              ...
              if (raw === '' && attrDef.autoMigrations.columnType.match(/^BIGINT$/i)) {
    ...
    var isFetchEnabled;
    if (query.meta && query.meta.fetch) { isFetchEnabled = true; }
    ...
    if (isFetchEnabled) {
      ...
      }, true, DryWLModel.identity, dryOrm);
      return phRecords;
    }//ﬁ
  },
