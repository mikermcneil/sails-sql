      ...
      pool.on('error', (err)=>{
        ...
        if (onUnexpectedFailure) {
          ...
          if (flaverr.parseError(err)) {
            err = flaverr({ raw: flaverr.parseError(err), message }, err);
          } else {
    ...
    try {
      if (connection.dialect === 'mysql') {
        ...
        return await new Promise((resolve, reject)=>{
          ...
          let mssqlRequest = new connection.manager.dbLibrary.Request(potentiallyAdjustedNativeQuery, (err, rowCount, rows)=>{
            if (err) {
            ...
            let result = rows;//TODO
            resolve({ result, meta });
          });//_∏_

          for (let idx in valuesToEscape) {
            mssqlRequest.addParameter(`p${idx}`, connection.manager.dbLibrary.TYPES.NVarChar, valuesToEscape[idx], {
              // ^TODO: are other types necessary?  (http://tediousjs.github.io/tedious/api-datatypes.html)
              length: Infinity//« see http://tediousjs.github.io/tedious/api-request.html#function_addParameter
            });
          }//∞
  ...
  parseNativeQueryError: function({nativeQueryError, meta}) {
    ...
    if (nativeQueryError.dialect === 'mysql') {
      ...
      if (nativeQueryError.code === 'EREQUEST' && (nativeQueryError.number === 208 || nativeQueryError.number === 3701)) {// https://docs.microsoft.com/en-us/previous-versions/sql/sql-server-2008-r2/cc645601%28v%3dsql.105%29
        ...
        // TODO: actually handle uniqueness errors
      } else {
        footprint = { identity: 'catchall' };
      }
      return { footprint, meta };
