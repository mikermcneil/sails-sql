...
var DRIVER = {
  ...
  createEachRecord: async function({query, connection, dryOrm}) {
    ...
    for (let qInfo of nativeQueries) {
      let nativeResult;
      try {
        nativeResult = await _runSqlQuery(qInfo.sql, qInfo.bindings, connection, query.meta, DRIVER);
      } catch (err) {
  ...
  updateRecords: async function({query, connection, dryOrm}) {
    // TODO: MAKE SURE WE'RE ACTUALLY PARSING CRITERIA CORRECTLY!!!!!
    var tableName = query.using;
    var DryWLModel = _.find(dryOrm.models, { tableName });
    var dialect = connection.dialect;
    var isFetchEnabled = (query.meta && query.meta.fetch) ? true : false;
    ...
    kChain = _applyWhereClauseToKnexChain(query.where, kChain, DryWLModel, dryOrm);
    kChain = kChain.update(query.valuesToSet);// « FUTURE: probably need to use 2nd arg here to implement optimized return tactic, so making that happen would take some refactoring.  BUT might be able to just chain on ".returning()" -- see knex docs when the time comes

    var kQuery = kChain.toSQL();
    var nativeResult;
    try {
      nativeResult = await _runSqlQuery(kQuery.sql, kQuery.bindings, connection, query.meta, DRIVER);
    } catch (err) {
      throw (!err.footprint) ? err
      : err.footprint.identity === 'notUnique' ? {notUnique: err}
      : err;
  ...
  destroyRecords: async function({query, connection, dryOrm}) {
    ...
    var kQuery = kChain.toSQL();
    var nativeResult = await _runSqlQuery(kQuery.sql, kQuery.bindings, connection, query.meta, DRIVER);
  ...
  findRecords: async function({query, connection, dryOrm}) {
    ...
    kChain = _applyWhereClauseToKnexChain(query.where, kChain, DryWLModel, dryOrm);
    kChain = kChain.limit(query.limit);
    kChain = kChain.offset(query.skip);
    for (let comparatorDirective of query.sort) {
  ...
  sumRecords: async function({query, connection, dryOrm}) {
    ...
    var kQuery = kChain.toSQL();
    var nativeResult = await _runSqlQuery(kQuery.sql, kQuery.bindings, connection, query.meta, DRIVER);
    return nativeResult.rows[0].VAL;
  },
...
function _applyWhereClauseToKnexChain (whereClause, kChain, DryWLModel, dryOrm){
  // TODO
  if (false) {
    console.log(dryOrm);
  }
