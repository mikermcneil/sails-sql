...
var DRIVER = {
  ...
  findRecords: async function({query, connection, dryOrm}) {
    ...
    var kQueryResult = await _runSqlQuery(kQuery.sql, kQuery.bindings, connection, query.meta, DRIVER);
    processNativeRecords(kQueryResult.rows, dialect, DryWLModel.identity, dryOrm);
    return kQueryResult.rows;
  },
  ...
  countRecords: async function({query, connection, dryOrm}) {
    ...
    var kQuery = kChain.toSQL();
    var kQueryResult = await _runSqlQuery(kQuery.sql, kQuery.bindings, connection, query.meta, DRIVER);
    var total = kQueryResult.rows[0].VAL;
    if (dialect === 'pg') {
  ...
  sumRecords: async function({query, connection, dryOrm}) {
    ...
    var kQuery = kChain.toSQL();
    var kQueryResult = await _runSqlQuery(kQuery.sql, kQuery.bindings, connection, query.meta, DRIVER);
    return kQueryResult.rows[0].VAL;
  },
