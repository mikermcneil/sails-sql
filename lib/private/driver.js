...
var DRIVER = {
  ...
  definePhysicalModel: async function({connection, tableName, columns, meta}) {
    switch (connection.dialect) {
      ...
      case 'oracledb':
      case 'sqlite3':
        let qInfo = (knex({ client: connection.dialect }).schema.createTable(tableName, (table)=>{
          // TODO finish (http://knexjs.org/#Schema-createTable)
          table.increments();//« remove this example
        }).toSQL())[0];
  ...
  dropPhysicalModel: async function({connection, tableName, meta}) {
    switch (connection.dialect) {
      ...
      case 'oracledb':
      case 'sqlite3':
        let qInfo = (knex({ client: connection.dialect }).schema.dropTable(tableName).toSQL())[0];
        try {
          await DRIVER.sendNativeQuery({
            connection,
            nativeQuery: qInfo.sql,
  ...
  setPhysicalSequence: async function({connection, sequenceName, sequenceValue, meta}) {
    switch (connection.dialect) {
      case 'pg':
        // TODO: special postgresql implementation
        throw new Error('Support for that dialect is incomplete...  (TODO)');
      case 'mysql':
        // No-op  (TODO verify this is a good idea -- though note that it is the current sails-mysql behavior)
        return;
      case 'mssql':
      case 'oracledb':
      case 'sqlite3':
        // TODO: general-case implementation
        throw new Error('Support for that dialect is incomplete...  (TODO)');
      default: throw new Error('Unsupported dialect');
    }
  },
