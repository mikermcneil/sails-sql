...
var DRIVER = {
  ...
  createEachRecord: async function({query, connection, dryOrm}) {
    ...
    if (!isCapableOfOptimizedReturnTactic) {
      ...
      if (fetchQResult.rows.length !== insertIds.length) { throw new Error(`When attempting to look up newly created records, the database returned the wrong number of rows.  Expected ${insertIds.length} row(s), but instead got ${fetchQResult.rows.length}: ${util.inspect(fetchQResult.rows, {depth:null})}`); }
