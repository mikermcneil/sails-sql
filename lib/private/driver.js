...
var DRIVER = {
  ...
  beginTransaction: async function({connection, meta}) {
    ...
    switch (connection.dialect) {
      ...
      case 'mssql':
      case 'sqlite3':
        await runSqlQuery('BEGIN', undefined, connection, meta, DRIVER);
        break;
      case 'oracledb':
        // For oracle, use START TRANACTION instead, because "BEGIN" has the
        // side-effect of causing auto-commit mode to become a thing
        await runSqlQuery('START TRANSACTION', undefined, connection, meta, DRIVER);
        break;
      default: throw new Error('Unsupported dialect');
    }
  ...
  commitTransaction: async function({connection, meta}) {
    ...
    switch (connection.dialect) {
      ...
      case 'oracledb':
      case 'sqlite3':
        await runSqlQuery('COMMIT', undefined, connection, meta, DRIVER);
        break;
      default: throw new Error('Unsupported dialect');
    }
  ...
  rollbackTransaction: async function({connection, meta}) {
    ...
    switch (connection.dialect) {
      ...
      case 'oracledb':
      case 'sqlite3':
        await runSqlQuery('ROLLBACK', undefined, connection, meta, DRIVER);
        break;
      default: throw new Error('Unsupported dialect');
    }
  ...
  createEachRecord: async function({query, connection, dryOrm}) {
    ...
    var nativeQueries = [];
    {
      ...
      for (let phValuesToSet of query.newRecords) {
        let queriesForSingleInsert;
        if (isFetchEnabled) {
          queriesForSingleInsert = k.returning('*').insert(phValuesToSet).toSQL();
          // TODO: do some testing to make sure "*" is actually going to work (https://knexjs.org/#Builder-returning)
          // FUTURE: Allow for configuring columns to return via .select() in Waterline
        } else {
          queriesForSingleInsert = k.insert(phValuesToSet).toSQL();
        }
        if (queriesForSingleInsert.length !== 1) { throw new Error(`This should never happen -- knex returned too many queries!  There should be only one, but instead got: ${util.inspect(queriesForSingleInsert, {depth:null})}`); }
        nativeQueries.push(queriesForSingleInsert[0]);
      }//∞
    }//∫

    // Run query, negotiating & rethrowing native query error, if applicable.
    let phRecords = isFetchEnabled ? [] : undefined;
    // if this is an insertion of ≥2 records...
    // • TODO: implement dialect-agnostic batching (for parity with knex core -- see https://knexjs.org/#Utility-BatchInsert and https://github.com/tgriesser/knex/blob/09eb12638c846822f06b4f6ab599ebcda6821d3f/src/util/batchInsert.js)
    // • TODO: make sure this fetch is properly preserving order, otherwise fall back (https://github.com/balderdashy/sails-mysql/blob/e9ca5d0d55fee6fbfe662597eb7dd291ffbcb323/helpers/private/query/create-each.js#L169-L184)
    // otherwise...
    // • TODO: implement basic optimized fetch
    for (let qInfo of nativeQueries) {
      let nativeQueryResult;
      try {
        nativeQueryResult = await runSqlQuery(qInfo.sql, qInfo.bindings, connection, query.meta, DRIVER);
      } catch (err) {
      ...
      if (isFetchEnabled) {
        phRecords.push(nativeQueryResult);
      }
    }//∞
  ...
  definePhysicalModel: async function({connection, tableName, columns, meta}) {
    ...
    for (let qInfo of nativeQueries) {
      await runSqlQuery(qInfo.sql, qInfo.bindings, connection, meta, DRIVER);
    }//∞
  },
...
async function runSqlQuery (sql, valuesToEscape, connection, meta, DRIVER){
  try {
    return await DRIVER.sendNativeQuery({ nativeQuery: sql, valuesToEscape, connection, meta });
  } catch (errOrSignal) {
