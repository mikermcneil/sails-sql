    ...
    for (let qInfo of nativeQueries) {
      ...
      if (isFetchEnabled) {
        if (isCapableOfOptimizedReturnTactic) {
          ...
          throw new Error('FUTURE: optimized "returning" tactic for dbs like postgresql and mssql');
        } else {
          let fetchQInfo = k(tableName).where({
            [DryWLModel.attributes[DryWLModel.primaryKey].columnName]: nativeQueryResult.insertId
          }).select('*').toSQL();
          let fetchQResult = await runSqlQuery(fetchQInfo.sql, fetchQInfo.bindings, connection, query.meta, DRIVER);
          // ^FUTURE: nicer error message for if this specific query fails for any reason
          if (fetchQResult.rows.length !== 1) { throw new Error(`When attempting to look up newly created records, the database returned the wrong number of rows.  Expected one row, but instead got ${fetchQResult.rows.length}: ${util.inspect(fetchQResult.rows, {depth:null})}`); }
          phRecords.push(fetchQResult.rows[0]);
        }
      }
    }//∞
