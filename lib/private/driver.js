...
var requireDbLibrary = (dialect)=>{
  switch (dialect) {
    ...
    case 'mssql':
      // Source code for Tedious (the MSSQL client library we're using):
      // https://github.com/tediousjs/tedious/tree/v5.0.3  (check the package.json of this repo to make sure that the version number in this link is still up to date with what we're actually using!!)
      //
 ...
 *
 * > This is a variable because we access it via closure from within its own
 * > functions (i.e. in order to call other driver methods).  This isn't really
 * > ideal or anything- it'd probably be better to extrapolate the shared logic
 * > into little utilities.  But this way, it does minimize the amount of code
 * > we have to maintain.  So there's that.
 *
 * @type {Dictionary}
 */
var DRIVER = {
  ...
  destroyManager: async function({manager, meta}) {
    ...
    if (manager.dialect === 'mysql') {
      // FUTURE: double-check whether we ought to call manager.pool.removeAllListeners('error') here
  ...
  sendNativeQuery: async function({connection, nativeQuery, valuesToEscape, meta}) {
    ...
    try {
      // First, if there are bindings, potentially adjust the query:
      // Replace any "?" with $1, $2, etc.  (Question marks are injected by knex
      // when it compiles queries, but documented usage of sendNativeQuery is
      // with the $1/$2/etc-style bindings.)  For more info, see:
      // sailsjs.com/docs/reference/waterline-orm/datastores/send-native-query
      //
      // Note that, below, depending on dialect, we may have to modify the query
      // yet again to change it to use dialect-specific syntax instead
      // (e.g. "?" for MySQL, "@p0"/"@p1"/etc. for MSSQL)
      //
      // > FUTURE: we could optimize this code a bit using meta.isUsingQuestionMarks
      // > But then we'd have to maintain that again, so we should make sure it'd
      // > actually be worth it first.
      // >
      // > And for historical context, see also:
      // > https://github.com/sailshq/machinepack-mysql/blob/8138628c495bf21a46e7e0a8c8a2e12ec2f07a20/lib/send-native-query.js#L115-L139
      // >
      // > FUTURE: (maybe?) We could also log a warning if any question marks were detected.
      // > For more info on that, see https://gist.github.com/mikermcneil/612fbb9bbe5b3dc3311a85110cec1427
      let potentiallyAdjustedNativeQuery;
      if (!valuesToEscape) {
        potentiallyAdjustedNativeQuery = nativeQuery;
      } else {
        let idx = 0;
        potentiallyAdjustedNativeQuery = nativeQuery.replace(/\?/g, (substr)=>{
          if (idx >= valuesToEscape.length) {
            idx++;
            return substr;// « leave original substr ("?") alone
          } else {
            let newSubstr = `$${idx+1}`;//« 1-indexed
            idx++;
            return newSubstr;
          }
        });//∞
      ...
      // console.log('initial potentiallyAdjustedNativeQuery',potentiallyAdjustedNativeQuery);

      if (connection.dialect === 'mysql') {
        return await new Promise((resolve, reject)=>{
          let potentiallyAdjustedValuesToEscape;
          if (!valuesToEscape) {
            potentiallyAdjustedValuesToEscape = valuesToEscape;
          } else {
            // Remember, $2 could come before $1 in the query!!! (x_x)
            potentiallyAdjustedValuesToEscape = [];
            potentiallyAdjustedNativeQuery = potentiallyAdjustedNativeQuery.replace(/\$[1-9][0-9]*/g, (substr)=>{
              let bindingNum = +( substr.slice(1) );
              if (bindingNum - 1 >= valuesToEscape.length) {
                return substr;// « leave original substr (e.g. "$3") alone
              } else {
                potentiallyAdjustedValuesToEscape.push(valuesToEscape[bindingNum-1]);
                return '?';
              }
            });//∞
          }//ﬁ
          // console.log('potentiallyAdjustedNativeQuery for MySQL',potentiallyAdjustedNativeQuery);
          // console.log('original valuesToEscape:  ', valuesToEscape);
          // console.log('potentiallyAdjustedValuesToEscape for MySQL:  ', potentiallyAdjustedValuesToEscape);
          connection.query({
            sql: potentiallyAdjustedNativeQuery,
            values: potentiallyAdjustedValuesToEscape
          }, (err, cbArg2, cbArg3)=>{
            if (err) {
        ...
        return await new Promise((resolve, reject)=>{
          if (valuesToEscape) {
            potentiallyAdjustedNativeQuery = potentiallyAdjustedNativeQuery.replace(/\$[1-9][0-9]*/g, (substr)=>{
              let bindingNum = +( substr.slice(1) );
              let idx = bindingNum - 1;
              if (idx >= valuesToEscape.length) {
                return substr;// « leave original substr (e.g. "$3") alone
              } else {
                return `@p${idx}`;//« 0-indexed
              }
            });//∞
          }//ﬁ
          // console.log('potentiallyAdjustedNativeQuery for MSSQL:  ', potentiallyAdjustedNativeQuery);
          // console.log('valuesToEscape for MSSQL:  ', valuesToEscape);
          ...
          for (let idx in valuesToEscape) {
            ...
            // console.log('• detailed "mssqlParamType":', mssqlParamType);
            // console.log(`• valuesToEscape[${idx}]  (${mssqlParamType.type}):`, valuesToEscape[idx]);
  ...
  parseNativeQueryError: function({nativeQueryError, meta}) {
    ...
    if (nativeQueryError.dialect === 'mysql') {
      ...
      if (nativeQueryError.code === 'EREQUEST' && (nativeQueryError.number === 208 || nativeQueryError.number === 3701)) {// https://docs.microsoft.com/en-us/previous-versions/sql/sql-server-2008-r2/cc645601%28v%3dsql.105%29
        ...
        // --o--o--o--o--o--o--o--o--o--o--o--o--o--o--o--o--o--o--o-<>
        footprint = { identity: 'noSuchPhysicalModel' };
      } else if (nativeQueryError.code === 'EREQUEST' && (nativeQueryError.number === 2601 || nativeQueryError.number === 2627)) {
        // --o--o--o--o--o--o--o--o--o--o--o--o--o--o--o--o--o--o--o-<>
        // message: 'Cannot insert duplicate key row in object 'dbo.the_foo' with unique index 'the_foo_the_beep_unique'. The duplicate key value is (1.55239e+012).'
        // code: 'EREQUEST',
        // number: 2601,
        // --o--o--o--o--o--o--o--o--o--o--o--o--o--o--o--o--o--o--o-<>
        // --o--o--o--o--o--o--o--o--o--o--o--o--o--o--o--o--o--o--o-<>
        // message: 'Violation of PRIMARY KEY constraint 'PK__user__3213E83F4D8D715C'. Cannot insert duplicate key in object 'dbo.user'. The duplicate key value is (98).'
        // code: 'EREQUEST',
        // number: 2627,
        // --o--o--o--o--o--o--o--o--o--o--o--o--o--o--o--o--o--o--o-<>
  ...
  beginTransaction: async function({connection, meta}) {
    ...
    switch (connection.dialect) {
      ...
      case 'sqlite3':
        ...
        break;
      case 'mssql': // « See https://docs.microsoft.com/en-us/sql/t-sql/language-elements/begin-transaction-transact-sql?view=sql-server-2017
        // TODO: finish this (see https://github.com/tediousjs/tedious/issues/891)
        // await runNativeQuery('SET IMPLICIT_TRANSACTIONS ON', undefined, connection, meta, DRIVER);
        await runNativeQuery('SET IMPLICIT_TRANSACTIONS ON; BEGIN TRAN', undefined, connection, meta, DRIVER);
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // CAREFUL- the BELOW DOESNT SEEM TO BE TRUE:
        // ```
        // Note that for MSSQL, we must first disable autocommit
        // (aka `SET IMPLICIT_TRANSACTIONS ON;`)
        // > More info:
        // > • https://docs.microsoft.com/en-us/sql/t-sql/statements/set-implicit-transactions-transact-sql?view=sql-server-2017
        // > • https://docs.microsoft.com/en-us/sql/odbc/reference/develop-app/setting-the-commit-mode?view=sql-server-2017
        // ```
        //
        // Instead here's how it works: We're still seeing weird errors.  With the "SET", we see:
        //     RequestError: Transaction count after EXECUTE indicates a mismatching number of BEGIN and COMMIT statements. Previous count = 2, current count = 1.
        // But without the "SET", we see:
        //      RequestError: Transaction count after EXECUTE indicates a mismatching number of BEGIN and COMMIT statements. Previous count = 0, current count = 1.
        //
        // So I dunno what's up.
        // I've been testing with:
        // ```
        // User.getDatastore().transaction(async(db)=>{ /* do nothing, for now */ }).log()
        // ```
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        break;
      case 'oracledb':
  ...
  commitTransaction: async function({connection, meta}) {
    ...
    switch (connection.dialect) {
      ...
      case 'sqlite3':
        ...
        break;
      case 'mssql':// « See https://docs.microsoft.com/en-us/sql/t-sql/language-elements/commit-transaction-transact-sql?view=sql-server-2017
        await runNativeQuery('COMMIT TRAN', undefined, connection, meta, DRIVER);
        break;
      default: throw new Error('Unsupported dialect');
    }
  ...
  rollbackTransaction: async function({connection, meta}) {
    ...
    switch (connection.dialect) {
      ...
      case 'sqlite3':
        ...
        break;
      case 'mssql':// « See https://docs.microsoft.com/en-us/sql/t-sql/language-elements/rollback-transaction-transact-sql?view=sql-server-2017
        await runNativeQuery('ROLLBACK TRAN', undefined, connection, meta, DRIVER);
        break;
      default: throw new Error('Unsupported dialect');
    }
  ...
  createEachRecord: async function({query, connection, dryOrm}) {
    ...
    var nativeQueries = [];
    {
      ...
      for (let phValuesToSet of query.newRecords) {
        ...
        // Handle MSSQL IDENTITY_INSERT stuff
        // > But only bother with this is phValuesToSet contains a primary key value
        // >
        // > This fixes the "Cannot insert explicit value for identity column in table '……' when IDENTITY_INSERT is set to OFF." error.
        // > For more, see https://github.com/tgriesser/knex/issues/2262#issuecomment-452065270
        // > And https://stackoverflow.com/questions/7063501/how-to-turn-identity-insert-on-and-off-using-sql-server-2008
        if (dialect === 'mssql') {
          let pkColumnName = DryWLModel.attributes[DryWLModel.primaryKey].columnName;
          // console.log('pkColumnName', pkColumnName);
          // console.log('phValuesToSet[pkColumnName]', phValuesToSet[pkColumnName]);
          if (phValuesToSet[pkColumnName] !== null && phValuesToSet[pkColumnName] !== undefined) {
            queryForSingleInsert.sql = (
              `SET IDENTITY_INSERT [${tableName}] ON; `+
              `${queryForSingleInsert.sql} `+
              `SET IDENTITY_INSERT [${tableName}] OFF;`
            );
            // console.log('Wrapped query: ', queryForSingleInsert.sql);
          }//ﬁ
        }//ﬁ
    ...
    if (isFetchEnabled) {
      // FUTURE: consider standardizing this in the future, but see https://github.com/balderdashy/sails-docs/commit/ca05d8a2bcbacda55940c5953c25195b7c0929c3 as well as https://github.com/balderdashy/sails-mysql/blob/e9ca5d0d55fee6fbfe662597eb7dd291ffbcb323/helpers/private/query/create-each.js#L169-L184
  ...
  updateRecords: async function({query, connection, dryOrm}) {
    ...
    try {
      ...
      if (err.footprint && err.footprint.identity === 'notUnique') {
        ...
        // Unlike the related code in the implementation of createEachRecord(),
        // there is no workaround: MSSQL just won't let you update the pk value
        // of a record.  For more info, see:
        // https://stackoverflow.com/a/19155816/486547
        if (dialect === 'mssql') {
          let pkColumnName = DryWLModel.attributes[DryWLModel.primaryKey].columnName;
          if (query.valuesToSet[pkColumnName] !== null && query.valuesToSet[pkColumnName] !== undefined) {
            throw flaverr({
              message: `Primary key (${pkColumnName}) cannot be updated when using Microsoft SQL Server.  `+err.message,
              raw: err
            });
          } else {
            throw err;
  ...
  findRecords: async function({query, connection, dryOrm}) {
    ...
    // FUTURE: For MSSQL, handle type compatibility error for TEXT type columns in order to better support that kind of usage (i.e. "The data types text and varchar are incompatible in the equal to operator.")
    // ^^^^in the mean time, varchar(any) seems to work for all use cases we've tried
      ...
      for (let column of columns) {
        if (column.autoIncrement) {
          ...
          switch (column.columnType.toLowerCase()) {
            ...
            case '_number':          computedColumnType = (
              connection.dialect === 'mssql' ? 'FLOAT'
              : 'REAL'
            ); break;
            case '_string':          computedColumnType = (// «FUTURE: Also use additional available information (like the "maxLength" validation rule) to make a better guess for the physical column type to use.
              column.unique ? (
                ['mssql', 'mysql'].includes(connection.dialect) ? 'VARCHAR(255)'
                : 'VARCHAR'
              )
              : (
                connection.dialect === 'mssql' ? 'VARCHAR(MAX)'//«https://stackoverflow.com/questions/4350060/where-clause-on-sql-server-text-data-type/14494505#comment4732276_4350060  (see also https://social.msdn.microsoft.com/Forums/sqlserver/en-US/4d9c6504-496e-45ba-a7a3-ed5bed731fcc/varcharmax-vs-varchar255?forum=sqlgetstarted)
                : connection.dialect === 'mysql' ? 'LONGTEXT'//«https://stackoverflow.com/questions/13932750/tinytext-text-mediumtext-and-longtext-maximum-storage-sizes
                : 'TEXT'
              )
            ); break;
            case '_boolean':         computedColumnType = (
              connection.dialect === 'mssql' ? 'BIT'
              : 'BOOLEAN'
            ); break;
            case '_json':            computedColumnType = (
              connection.dialect === 'mssql' ? 'VARCHAR(MAX)'
              : connection.dialect === 'mysql' ? 'LONGTEXT'
              : 'JSON'
            ); break;
            case '_ref':             computedColumnType = (
              connection.dialect === 'mssql' ? 'VARCHAR(MAX)'
              : connection.dialect === 'mysql' ? 'LONGTEXT'
              : 'TEXT'
            ); break;
            ...
            case '_numberkey':       computedColumnType = (
              'INTEGER'
            ); break;
            case '_stringkey':       computedColumnType = (
              ['mssql', 'mysql'].includes(connection.dialect) ? 'VARCHAR(255)'
              : 'VARCHAR'
            ); break;
            case '_numbertimestamp': computedColumnType = (
              'BIGINT'
            ); break;
            case '_stringtimestamp': computedColumnType = (
              ['mssql', 'mysql'].includes(connection.dialect) ? 'VARCHAR(255)'
              : 'VARCHAR'
            ); break;

            // Misc:
            default:                 computedColumnType = (
              column.columnType
            );
          }
