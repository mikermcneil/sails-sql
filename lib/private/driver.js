...
var DRIVER = {
    ...
    for (let qInfo of nativeQueries) {
      ...
      if (isFetchEnabled) {
        if (isCapableOfOptimizedReturnTactic) {
          // FUTURE: handle this
          // TODO: do some testing to make sure "*" we used above is actually going to work (https://knexjs.org/#Builder-returning)
          // throw new Error('FUTURE: Not yet implemented optimized "returning" tactic for dbs like postgresql and mssql - see nativishResult: '+nativishResult);
          phRecords = nativishResult.rows;
        } else {
  ...
  findRecords: async function({query, connection, dryOrm}) {
    ...
    switch (dialect) {
      ...
      case 'mssql':
        rows = nativishResult.rows;
        console.log('native result for find:', nativishResult);
        break;
