...
module.exports = function buildWaterlineAdapter(driverImpl) {
  ...
  var runStdAdapterMethod = prepareStdAdapterMethodRunner(registeredDsEntries, ƒ);
    ...
    registerDatastore: (dsConfig, physicalModelsReport, done)=>{
      (async()=>{
        ...
        for (let tableName in physicalModelsReport) {
          ...
          for (let attrName in dryModel.attributes) {
            let attrDef = dryModel.attributes[attrName];
            attrDef.autoMigrations = attrDef.autoMigrations || {};
            //^^ This is just so we don't have to do a bunch of checks later in the adapter.
          }
 ...
 */
function prepareStdAdapterMethodRunner(registeredDsEntries, ƒ) {
  /**
