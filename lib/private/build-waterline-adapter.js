  ...
  var adapter = {
    ...
    create: (datastoreName, query, done)=>{
      (async()=>{
        ...
        var connection = (query.meta && query.meta.leasedConnection) || undefined;
        if (!connection) {
          // TODO: otherwise, obtain a connection using dsEntry.manager
        }

        await ƒ.createRecord(query, connection, dryOrm)
        .intercept(async(err)=>{
          // TODO
        });
