  ...
  for (let methodName in driverImpl) {
    ...
    let nmDef = Object.assign(_.cloneDeep(abstractDef), {
      fn: async(inputs, exits)=>{
        // FUTURE: If method implementation is not an AsyncFunction, then don't
        // use `async` or `await` here (for increased performance).
  ...
  var adapter = {
    ...
    registerDatastore: (datastoreConfig, physicalModelsReport, done)=>{
      (async()=>{
        ...
        // Perform adapter-agnostic validation of connection URL
        try {
          if (!datastoreConfig.url) { throw new Error('Missing `.url`'); }
          // FUTURE: much more  (maybe use generic impl from sails-mongo)
        } catch (err) {
          return done(flaverr({ message: 'Invalid datastore configuration for '+adapterDisplayPhrase+'.  '+err.message+'\n [?] https://sailsjs.com/config/datastores#?the-connection-url', code: 'E_MALFORMED_DATASTORE_CONFIG' }, err));
        }
        ...
        var manager = (
          await ƒ.createManager(
            ...
            (unexpectedFailureErr)=>{
              console.warn(
                'Warning from '+adapterDisplayPhrase+': An unhandled error was emitted by the database manager/pool or one of its active db connections. ', unexpectedFailureErr
              );
            },
            datastoreConfig
          )
          .intercept('malformed', (err)=>flaverr({ message: 'Invalid datastore configuration for '+adapterDisplayPhrase+'.  '+err.message+'\n [?] https://sailsjs.com/config/datastores#?the-connection-url', code: 'E_MALFORMED_DATASTORE_CONFIG' }, err))
          .intercept('failed', (err)=>flaverr({ message: 'Error creating new manager/pool using '+adapterDisplayPhrase+'.  '+err.message,  code: 'E_FAILED_CREATING_DATASTORE_MANAGER' }, err))
          .intercept((err)=>flaverr({ message: 'Error creating new manager/pool using '+adapterDisplayPhrase+'.  '+err.message }, err))
