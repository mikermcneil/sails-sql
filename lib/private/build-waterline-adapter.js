  ...
  var adapter = {
    ...
    create: (datastoreName, query, done)=>{
      (async()=>{
        ...
        var connection;
        if (query.meta && query.meta.leasedConnection) {
          wasConnectionProvided = true;
          connection = query.meta.leasedConnection;
        } else {
          connection = (
            await ƒ.getConnection(dsEntry.manager, query.meta)
            .intercept('failed', (err)=>err.raw.error)
          ).connection;
        }

        var temporarilyToleratedErr;
        var result = await ƒ.createRecord(query, connection, dryOrm)
        .tolerate((err)=>{
          temporarilyToleratedErr = err;
        });

        if (!wasConnectionProvided) {
          await ƒ.releaseConnection(connection, query.meta)
    ...
    createEach: (datastoreName, query, done)=>{
      (async()=>{
        runAdapterQueryMethod(registeredDsEntries[datastoreName], query);
      })().then((result)=>{ done(undefined, result); }).catch((err)=>{ done(err); });
    },
...
async function runAdapterQueryMethod(dsEntry, query) {
